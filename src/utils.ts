import { Disc, formatTimeFromFrames, Encoding, getTracks} from 'netmd-js';
...
export function getAvailableCharsForTitle(disc: Disc){
    const cellLimit = 255; 
    // see https://www.minidisc.org/md_toc.html
    const fixLength = (len: number) => Math.ceil(len / 7);

    let usedCells = 0;
    usedCells += fixLength(disc.fullWidthTitle.length * 2);
    usedCells += fixLength(disc.title.length);
    for(let trk of getTracks(disc)){
        usedCells += fixLength((trk.fullWidthTitle?.length ?? 0) * 2);
        usedCells += fixLength(trk.title?.length ?? 0);
    }
